# Estudos de Holandês - Inburgering A2

Este repositório vai ser a nossa base para organizar seus estudos de holandês focados no exame de **inburgering nível A2**.

## Estrutura

- [00-estudar-com-chatgpt.md](./00-estudar-com-chatgpt.md)
- [DUTCH_A2_MASTER_REVIEW.md](./DUTCH_A2_MASTER_REVIEW.md)
- [01-plano-de-estudos/README.md](./01-plano-de-estudos/README.md)
- [02-vocabulario/temas-do-dia-a-dia.md](./02-vocabulario/temas-do-dia-a-dia.md)
- [02-vocabulario/alta-frequencia-a2.md](./02-vocabulario/alta-frequencia-a2.md)
- [02-vocabulario/licoes-11-a-20-temas-a2.md](./02-vocabulario/licoes-11-a-20-temas-a2.md)
- [03-gramatica/pontos-importantes-a2.md](./03-gramatica/pontos-importantes-a2.md)
- [04-frases-uteis/conversacao-basica.md](./04-frases-uteis/conversacao-basica.md)
- [05-escrita/modelos-de-texto.md](./05-escrita/modelos-de-texto.md)
- [06-pratica/lista-de-revisao.md](./06-pratica/lista-de-revisao.md)
- [06-pratica/estrategias-de-prova-a2.md](./06-pratica/estrategias-de-prova-a2.md)
- [07-exercicios/README.md](./07-exercicios/README.md)
- [07-exercicios/01-leitura.md](./07-exercicios/01-leitura.md)
- [07-exercicios/02-audicao.md](./07-exercicios/02-audicao.md)
- [07-exercicios/03-fala.md](./07-exercicios/03-fala.md)
- [07-exercicios/04-escrita.md](./07-exercicios/04-escrita.md)
- [07-exercicios/05-simulado-leitura-a2-estilo-exame.md](./07-exercicios/05-simulado-leitura-a2-estilo-exame.md)
- [08-simulados-oficiais-governo/README.md](./08-simulados-oficiais-governo/README.md)

## Como usar

1. Estude um tema de vocabulário por vez.
2. Revise um ponto de gramática junto com frases reais.
3. Escreva pequenas respostas usando os modelos.
4. Marque o que já domina na lista de revisão.

## Como estudar comigo aqui no chat

Use o arquivo [00-estudar-com-chatgpt.md](./00-estudar-com-chatgpt.md) como seu workflow principal.

A ideia é simples:

1. você escolhe um tema ou skill
2. eu preparo a revisão, explicação ou exercício
3. você responde aqui no chat
4. eu corrijo, explico os erros e ajusto o próximo passo

## Arquivo central de revisão

O arquivo [DUTCH_A2_MASTER_REVIEW.md](./DUTCH_A2_MASTER_REVIEW.md) é o seu material principal de revisão ampla. Ele é útil para:

- revisão rápida antes de estudar
- consolidar vocabulário de alta frequência
- revisar contrastes importantes como `want` vs `omdat`
- treinar leitura para o estilo do exame

## Vocabulário por temas

Além dos arquivos básicos de vocabulário, o arquivo [02-vocabulario/licoes-11-a-20-temas-a2.md](./02-vocabulario/licoes-11-a-20-temas-a2.md) consolida os capítulos mais temáticos do A2:

- clima
- transporte
- animais
- tempo e horários
- esporte
- lazer/restaurante/cultura
- férias e viagem
- natureza
- cultura holandesa
- política e regras

## Simulados oficiais do governo

A seção [08-simulados-oficiais-governo](./08-simulados-oficiais-governo/README.md) consolida o vocabulário e os padrões dos simulados oficiais/reais de leitura A2 do governo.

Ela contém:

- vocabulário completo por temas reais de prova;
- padrões de leitura, gramática funcional e pegadinhas;
- banco de 60 questões no estilo A2 com gabarito.

Essa é a seção mais importante para leitura prática, porque usa temas muito próximos dos textos oficiais: trabalho, escola, saúde, transporte, serviços, eventos, horários, regras, inscrições e mudanças temporárias.

## Exercícios e simulados

O arquivo [07-exercicios/05-simulado-leitura-a2-estilo-exame.md](./07-exercicios/05-simulado-leitura-a2-estilo-exame.md) contém um simulado autoral de leitura A2 com 25 perguntas, inspirado no formato do exame.

Para treino mais próximo dos exemplos oficiais, use também o banco em [08-simulados-oficiais-governo/04-banco-de-questoes.md](./08-simulados-oficiais-governo/04-banco-de-questoes.md).

## Foco do nível A2

No A2, a meta não é falar perfeito. A meta é conseguir:

- se apresentar
- falar da rotina
- marcar horário
- entender instruções simples
- escrever mensagens curtas
- conversar sobre trabalho, casa, saúde e compras

## Próximos passos sugeridos

- completar vocabulário com palavras que você realmente usa no dia a dia
- adicionar mais exercícios com respostas
- criar simulados curtos por tema
- registrar progresso na [lista de revisão](./06-pratica/lista-de-revisao.md)
