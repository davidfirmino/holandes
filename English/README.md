# Dutch A2 Study Notes — English Version

This directory contains English-only study material for Dutch A2 / Inburgering preparation.

The main repository is written mostly in Portuguese because it was built for a Brazilian Portuguese speaker. This `English/` mirror keeps the same study logic but uses English for explanations, translations, strategies, answer keys, and notes.

> Important: Dutch example texts, Dutch vocabulary, and Dutch exam-style exercises remain in Dutch, because they are the actual language being studied. The explanations and translations are in English only.

## Files in this directory

- [Dutch A2 Master Review — English](./DUTCH_A2_MASTER_REVIEW_EN.md)
...
At A2 level, the goal is not perfect Dutch. The goal is to understand practical, everyday language in contexts such as:
...
The [Official Government Simulations — A2 Reading](./official-government-simulations/README.md) section is the most exam-realistic reading block in this English directory.

It includes:

- full vocabulary by official-style topic;
- reading patterns, functional grammar, and traps;
- a 60-question official-style A2 reading question bank with answer key.

Use this section to practice practical texts about work, school, healthcare, transport, appointments, services, neighborhood events, schedules, deadlines, rules, registrations, and temporary changes.
...
- `geen` — no / not any
- `alleen` — only
- `vanaf` — from / starting at
- `voor` / `vóór` — before
- `tot` — until
- `tot en met` — up to and including
- `uiterlijk` — no later than / at the latest
- `ongeveer` — approximately
- `precies` — exactly
- `verplicht` — required / compulsory
- `verboden` — forbidden
- `toegestaan` — allowed
- `gratis` — free
- `gesloten` — closed
...
- `op tijd` — on time
- `te laat` — late
- `hoeft niet` — does not have to
- `mag niet` — may not / is not allowed
- `moet wel` — does have to / must indeed
